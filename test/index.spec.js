...
describe("stock_trades_api_hard", () => {
  ...
  it("should create a new trade of sell type", async () => {
    const response = await chai
      .request(server)
      .post("/trades")
      .send(user23_sell_AAC);
    response.should.have.status(201);
    delete response.body.id;
    response.body.should.eql(user23_sell_AAC);
  });

  it("should return 400 if the payload has an invalid type", async () => {
    const payload = { ...user23_buy_ABX, type: "test" };
    const response = await chai.request(server).post("/trades").send(payload);
    response.should.have.status(400);
  });

  it("should return 400 if the number of shares exceeds the limit", async () => {
    const payload = { ...user23_buy_ABX, shares: 102 };
    const response = await chai.request(server).post("/trades").send(payload);
    response.should.have.status(400);
  });

  it("should return 400 if the number of shares is below the limit", async () => {
  ...
  it("should fetch all buy trades for a user", async () => {
    ...
    const response = await chai
      .request(server)
      .get("/trades?user_id=23&type=buy");
    response.should.have.status(200);
  ...
  it("should fetch all sell trades for a user", async () => {
    ...
    const response = await chai
      .request(server)
      .get("/trades?user_id=23&type=sell");
    response.should.have.status(200);
